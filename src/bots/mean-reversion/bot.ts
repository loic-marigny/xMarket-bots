...
async function recordWealthSnapshot(
  ...
  try {
    ...
    console.debug("[Mean Reversion Bot] Recorded wealth snapshot", { cash, stocks, total });
  } catch (err) {
    console.error("[Mean Reversion Bot] Failed to record wealth snapshot", err);
  }
}
...
async function main() {
  ...
  console.log(`[Mean Reversion Bot] Authenticated as UID ${uid}`);

  const db = getFirestore(app);
  const supabase: AppSupabaseClient = createClient(supabaseUrl, supabaseAnonKey);

  const closes = await fetchLatestCloses(supabase, symbol, 40);
  if (closes.length < 20) {
    console.log("[Mean Reversion Bot] Not enough price history, exiting.");
    return;
  }
  ...
  console.log(
    `[Mean Reversion Bot] Latest=${closes.at(-1)}, Cash=${cash}, Qty=${qtyHeld}`,
  );
  ...
  console.log(`[Mean Reversion Bot] Logic decision: ${logicDecision.action} (${logicDecision.reason ?? "no reason provided"})`);

  if (logicDecision.action === "buy" && cash >= closes.at(-1)! * lotSize) {
    await submitSpotOrder(db, { uid, symbol, side: "buy", qty: lotSize, fillPrice: closes.at(-1)! });
  } else if (logicDecision.action === "sell" && qtyHeld >= lotSize) {
    await submitSpotOrder(db, { uid, symbol, side: "sell", qty: lotSize, fillPrice: closes.at(-1)! });
  } else {
    console.log("[Mean Reversion Bot] No order submitted.");
  }
...
main()
  .then(() => {
    console.log("[Mean Reversion Bot] Run completed.");
    process.exit(0);
  })
  .catch((err) => {
    console.error(err);
