...
async function fetchLatestCloses(
  ...
  const { data, error } = await withRetries(`Trend Follower history ${symbol}`, async () =>
    supabase
...
async function recordWealthSnapshot(
  ...
  try {
    ...
    console.debug("[Trend Follower Bot] Recorded wealth snapshot", { cash, stocks, total });
  } catch (err) {
    console.error("[Trend Follower Bot] Failed to record wealth snapshot", err);
  }
}
...
async function main() {
  ...
  const botEmail = requiredEnv("BOT_TREND_EMAIL");
  const botPassword = requiredEnv("BOT_TREND_PASSWORD");

  const symbol = (optionalEnv("BOT_TREND_SYMBOL") ?? "AAPL").toUpperCase();
  const lotSize = positiveNumberEnv("BOT_TREND_QTY", 1);

  console.log(`[Trend Follower Bot] Target symbol: ${symbol}, Lot size: ${lotSize}`);

  const app = initializeApp(firebaseConfig);
  const auth = getAuth(app);
  const cred = await signInWithEmailAndPassword(auth, botEmail, botPassword);
  const uid = cred.user.uid;
  console.log(`[Trend Follower Bot] Authenticated as UID ${uid}`);

  const db = getFirestore(app);
  const supabase: AppSupabaseClient = createClient(supabaseUrl, supabaseAnonKey);

  const closes = await fetchLatestCloses(supabase, symbol, 220);
  if (closes.length < 200) {
    console.log("[Trend Follower Bot] Not enough price history, exiting.");
    return;
  }
  ...
  console.log(`[Trend Follower Bot] Cash=${cash}, Qty=${qtyHeld}`);
  ...
  console.log(`[Trend Follower Bot] Logic decision: ${logicDecision.action} (${logicDecision.reason ?? "no reason provided"})`);
  ...
  if (logicDecision.action === "buy" && cash >= latest * lotSize) {
    ...
    console.log("[Trend Follower Bot] No order submitted.");
  }
...
main()
  .then(() => {
    console.log("[Trend Follower Bot] Run completed.");
    process.exit(0);
  })
  .catch((err) => {
    console.error(err);
